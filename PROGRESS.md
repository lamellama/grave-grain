...
- **Current work:** post-MVP **Authorized backlog** (user greenlit). ✅ DONE so far: (a) revised death model (extreme→dissolve / quiet→corpse / bite→turn, T1–T6); (b) zombie ladder-climb (crowd piles & climbs walls); (c) plant-a-seed foliage growth (SAPLING→bush, deterministic/chunk-safe). **NEXT: Warmth + camp vertical slice** (Warmth need + temperature + fire-as-warmth + Shelter structure — the original 'shelter/water/fire to keep warm' ask), then cooperative base-building (Builder role), then GDD §14 Beyond. All invariants (GATE, chunk byte-equivalence, win/lose, no-tunnel) green throughout.
