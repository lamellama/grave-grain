...
- **Current work:** post-MVP **Authorized backlog** (user greenlit). ✅ DONE so far: (a) revised death model (extreme→dissolve / quiet→corpse / bite→turn); (b) zombie ladder-climb; (c) plant-a-seed foliage growth (deterministic/chunk-safe); (d) **Warmth + camp vertical slice** (warmth need + roof-based open shelter + fire-warmth + cold-start camp; survivors warm under a roof, leave for water, freeze→corpse if stranded). **NEXT: cooperative base-building (Builder/Hauler role — survivors construct queued structures, GDD §6.2)**, then GDD §14 Beyond (Diggers/Fisherman roles, iron tier, herd dynamics, dual-edge spawns, tree reproduction, day/night, weather — prioritize with user). All invariants (GATE, chunk byte-equivalence, win/lose, no-tunnel) green throughout.
