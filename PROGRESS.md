...
- **GATE:** cleared. **Phase 10 carry-forwards / balance:** (a) ~~dpr putImageData corner bug~~ — FIXED (user-reported): render in CSS px (canvas backing = floor(rect.w/h), no dpr scale) so ImageData == backing store. Possible later refinement: crisp-retina device-px rendering. (b) Balance: zombie spawn edge is ~920 cells from the colony → sparse early combat; consider closer margin or faster pursuit. (c) miner targets any exposed STONE/WALL — fine in worldgen (DIRT surface) but a player wall at surface could be mined; revisit if it bites.
