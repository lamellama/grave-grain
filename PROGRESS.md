...
VS-2 Warmth · status · warmth-half ALREADY DONE (prior p12 warmth pass: needs.warmth, WARMTH_RATE/RESTORE/THRESHOLD, isSheltered, nearWarmth, seekWarmth, freeze→corpse death, warmth HUD bar; VS-1 T4 wired cold gate to dynamic weather). (A) wetness need + WET_WARMTH_MULT ✅ DONE (T-A, commit below). REMAINING for VS-2: (B) graded local effective temp + SNOW_CONTACT_PENALTY/WARMTH_SAMPLE_TICKS, (C) campfire (managed contained fire) + CAMPFIRE_FUEL, plus the VS-2 HUD bits (wet icon via WET_ICON_THRESHOLD, freeze toast — toast already exists). **NEXT: T-B (graded local effective temp).**
