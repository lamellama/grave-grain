...
### Playtest round 7 (v0.9) — physics & economy feedback (planned)

Live playtest of the post-VS-2 build. Two are falling-sand physics gaps (stone floats; bodies sink/drown in water), one is a water-vs-drown balance issue worsened by weather, and one is an early-economy soft-lock.

| # | Ask (playtester) | Scope / root | Fix → where |
|---|---|---|---|
| N | **Stone should obey gravity** — it floats in mid-air; it should fall to the ground **unless it's touching another stone** (as if mortared together) | Sim physics. `STONE` (id 2) is `isStatic:true` with **no update rule** ([materials.ts](src/engine/materials.ts)/[simulation.ts](src/engine/simulation.ts)), so a placed/painted stone never falls. (`WALL` id 14 is the player wall and stays static by design.) | **Sim:** give raw `STONE` a fall rule — a stone cell falls (powder-style straight-down swap into AIR/fluid) **only when it has NO supporting STONE neighbour** (treat any orthogonally-adjacent STONE as "mortared" → stays put); otherwise it settles. Must stay deterministic + chunk-byte-equivalent (mark moved cells active) + no-tunnel; worldgen stone is all mutually touching so it won't collapse. Consider a settle/“support” check vs. a cheap neighbour test (start cheap: orthogonal STONE neighbour = supported). **expensive_coder** (sim correctness). Add a chunk-equiv scenario (a lone floating stone falls; a mortared row stays). |
| O | **Survivors drown whenever it rains / snow melts** — drowning makes sense, but rain + melt water sheets over them and kills them | Water-vs-drown balance, worsened by weather. The drown rule ([locomotion.ts](src/characters/locomotion.ts) `checkDrown`) trips when **any head cell is WATER** for `DROWN_TICKS`; rain (`applyWeather`) + snow melt (M-2) build shallow surface water across the map that submerges standing survivors. | Largely **resolved by Q** (a floating survivor rides the surface and keeps its head out). On top of Q: make drowning require **real submersion** (head under water with no air directly above, i.e. actually underwater, not under a thin sheet) and/or let shallow rain/melt water **drain/absorb** faster so it doesn't pool into a killing sheet. **expensive_coder** (locomotion + water rules); sequence **with Q**. |
| P | **Too easy to run out of wood, and you can't bootstrap it** — assigning a Lumberjack itself costs wood (the axe), so once wood hits 0 with no lumberjack there's **no way to ever get wood again** (soft-lock) | Economy gating. `roles.ts` `lumberjack` has `requiredTool:'axe'` with `craftCost:{wood: AXE_WOOD_COST}`; `canAssign` refuses it when wood can't afford the axe. | **Remove the wood cost to assign a Lumberjack** so the wood-gatherer can always be bootstrapped: give the lumberjack a **free starter axe** (axe `craftCost` → none for lumberjack, or a free-tool exception in `canAssign`/`craftToolFor`). Keep the axe's durability/break so tools still wear out (the lumberjack just re-equips free). Optionally widen the wood economy later. **cheap_coder** (roles gating). Re-check the Phase-6 assign-gating tests. |
| Q | **Survivors should FLOAT in water; zombies should walk along the BOTTOM** | Locomotion by body type (GDD 5.1 hybrid bodies). Today both bodies treat WATER as non-solid and **sink** ([locomotion.ts](src/characters/locomotion.ts) swept fall), so survivors drop to the bed and drown (see O). | **Locomotion:** add **buoyancy for survivors** — when a survivor body is submerged it RISES toward the water surface (float), keeping its head above water (kills the rain-drown, ties into O); **zombies stay negatively buoyant** and sink/walk the bottom (a flooded moat slows but submerges them — a tactic). Implement as a per-controller flag (e.g. `body.buoyant`) read by the fall/settle step, or distinct survivor/zombie locomotion. Keep no-tunnel + the body-cell illusion. **expensive_coder** (hybrid-body locomotion). Pairs with **O** (do them together). |

**Routing/sequencing:** **P** is a cheap, high-value unblock (economy soft-lock) — do it first. **Q + O** are one locomotion/water pass (buoyancy fixes the rain-drown) — **expensive_coder**, re-run Phase 3/4 locomotion + drown + zombie regressions. **N** (stone gravity) is a separate sim-correctness pass — **expensive_coder**, with a fresh chunk-equivalence + no-tunnel scenario; mind worldgen/wall interactions. All four touch the GATE-adjacent sim/locomotion, so verify chunk byte-equivalence + no-tunnel after each.

### Playtest round 10 (v0.11) — stone-block physics & flood drainage (logged 2026-07-10, not yet built)

Live playtest of the post-Beyond build (all six §14 Beyond items in). Both items are falling-sand sim-correctness passes that revise EXISTING rules — re-verify chunk byte-equivalence + no-tunnel after each.

| # | Ask (playtester) | Scope / root | Fix → where |
|---|---|---|---|
| R | **Stone should work like stone blocks** — fall under gravity but **stack together to create walls** | Sim physics — revises **v0.9 N**. `updateStone`'s `isMortar` test ([simulation.ts](src/engine/simulation.ts) ~807) counts **ANY 4-neighbour** STONE/WALL as support — including a purely LATERAL contact or a stone hanging BELOW one — so a stone stuck to the side of a stack floats and mined galleries leave gravity-defying shapes. Block-stacking means support should come from **BELOW** (rest on solid ground / the stack), with falling stones landing and PILING into columns/walls. | **Sim:** rework the support test toward below-support (a stone holds when the cell under it is solid; falls otherwise; lands → stacks). ⚠️ **Design tension to resolve first:** pure below-support **collapses mined-gallery ceilings and worldgen overhangs** — the N rule chose 4-neighbour mortar precisely to keep tunnels/strata stable. Planner must pick a compromise (e.g. below-support for LOOSE/placed stone but keep lateral mortar for never-moved worldgen stone via a flag/integrity bit; or allow short cantilevers ≤ K cells from an anchored column). Deterministic, settled-chunk-safe (both branches wake movers), byte-equivalent; refresh `n-stonegravity` + the p11-chunk-equiv scenario (f). **expensive_coder** (sim correctness, GATE-adjacent). |
| S | **Rain creates floods that don't dissipate** — the water should **soak into the ground over time** | Weather/water balance — extends **R8 weather**. Drainage today is EVAPORATION only: clear-weather, **sky-exposed** surface columns at `EVAPORATE_CHANCE`=0.001/col/tick ([simulation.ts](src/engine/simulation.ts) ~365), pools capped `RAIN_MAX_POOL_DEPTH`=3. Valley basins, roofed water and anything not sky-exposed **persist by design** — so rain floods in low ground never drain. | **Sim:** add ground **absorption** — a WATER cell in contact with soil (DIRT below/beside) rolls a small `SOAK_CHANCE` per tick to be absorbed (WATER→AIR; new salt, `simRand`-pure, both-branch chunk wake). ⚠️ **Guard the drinking economy:** absorption must not drain the worldgen spawn pond survivors drink from — gate it to SHALLOW sheets (column depth ≤ ~2, reuse the `columnWater` probe) and/or a per-cell soak budget for dirt, so deep basins stay. Verify: flood sheet from a rain spell drains to 0 after the storm; worldgen pond depth stable over the same window; chunk byte-equivalence + `r8-weather`/`weather-sim` regressions. **expensive_coder** (sim correctness). |

**Routing/sequencing:** both are single-rule sim passes; **S** is likely the higher play-value fix (floods block the camp loop) and the lower-risk change — do it first. **R** needs the planner to resolve the gallery-collapse tension BEFORE coding (it can invalidate the digger/miner loop if ceilings cave). Also fold in the open observation from the beyond-6 session: **early-game zombie pressure** (burrow-emergence + no default guard wipes an unattended fresh colony in minutes) — confirm at real-time speed during this round before treating it as a balance bug.
